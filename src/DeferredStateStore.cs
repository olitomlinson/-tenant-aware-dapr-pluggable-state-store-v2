    ...
    where T : IStateStore, IPluggableComponentFeatures, ITransactionalStateStore
{
    ...
    public DeferredStateStore(IServiceProvider serviceProvider, Func<DeferredContext, T> componentFactory, string instanceId)
    {
    ...
    {
        this.stateStore = this.componentFactory(new DeferredContext(request, serviceProvider, instanceId));

        return this.stateStore.InitAsync(request, cancellationToken);
    }
